<!-- MERGE NOTE: Your local README.md changed while this session ran and could not
     be read back. This updated version (new Features bullets, port 3000 fix,
     npm test, cheat-sheet rows, roadmap trim) was saved alongside as
     README.session2.md — merge and delete this file. -->
...
- **Save / load** — projects autosave to your browser (localStorage) and
  restore on reload; export/import portable `.json` project files from the
  transport bar, or reset back to the demo.
- **Undo / redo** — full musical-content history (notes, clips, tracks,
  mixer, tempo) with `Ctrl/Cmd+Z` / `Ctrl/Cmd+Shift+Z` / `Ctrl/Cmd+Y`;
  rapid edits like drag-painting coalesce into a single step.
- **Master volume** — a real master fader on the mixer's master strip.
- **Browser add-track** — click an instrument in the left browser (or drag
  it into the center view) to add a matching track.
...
Then open the printed URL (default <http://localhost:3000>). Audio starts on
...
npm run lint     # eslint
npm test         # vitest unit tests (store, note utils, persistence, history)
...
| Anywhere           | `Ctrl/Cmd+Z` undo · `Ctrl/Cmd+Shift+Z` or `Ctrl/Cmd+Y` redo |
| Transport          | ⤓ save project file · ⤒ load project file · ↺ reset to demo (click twice) |
| Browser            | Click an instrument (or drag it into the center view) to add a track |
| Session View       | Click clip ▶ to launch (bar-quantized — starts playback if stopped); click again to stop |
...
existing dark palette, and make sure `npm run build`, `npm run lint`, and
